export class QueenAttack {
  constructor(position = {}) { //constructor accepts an obj param
  ...
  toString() { //toString() method is called in 2 tests
    this.setQueens();
    let boardStr = "";
  ...
  canAttack() {
    let attack = false
    if (this.white[0] === this.black[0] || this.white[1] === this.black[1]) {//checks rows and columns, if the x axis (index[0]) or y axis (index[1]) are the same they share a row or column
      return attack = true;
    } else if (//checks diagonals. If diagonally alligned, the sum or difference in cooridnates will be the same
